#Persona que alquila el vehiculo
class Cliente:

    def __init__(self, dni: str, nombre_completo: str, edad: int, carnets: list, direccion: str = '', descuento: float = 0, premium: bool = False, gastado_premium: float = 0, total_gastado: float = 0, total_ahorrado: float = 0) -> None:
        self.dni = dni
    ...
    #Modifica el total gastado
    @total_gastado.setter
    def total_gastado(self, valor: float) -> None:
        self._total_gastado = valor

    #Obtiene el gasto que cuenta para ser premium
    @property
    def gastado_premium(self) -> float:
        return self._gastado_premium

    #Modifica el gasto premium
    @gastado_premium.setter
    def gastado_premium(self, valor: float) -> None:
        self._gastado_premium = valor

    #Indica si el cliente tiene nivel premium
    @property
    def premium(self) -> bool:
        return self._premium

    #Instancia la clase directamente con sus atributos
    @classmethod
    def alta_Cliente(cls, dni, nombre_completo, edad, carnets, direccion='', descuento=0):
        return cls(dni, nombre_completo, edad, carnets, direccion, descuento)
    ...
    #Sobrecarga del operador += para anadir carnets
